...
You will never need to enter a username or password, or complete a
captcha to "prove you are human" or do any kind of manual
authentication. It will "just work".
*** Trusted
...
Now make a program that wraps a given pubkey in a delegating script.
#+begin_src kcats :results code
  "foo" bytes key [secret] unassign ;; the public key (hardcoded in real world use)
  [[sink ;; css cs pk
  [[bytes hash] [shield dip] decorated ;; css csh cs pk
...
#+begin_src kcats
  [[[public #b64 "NNJledu0Vmk+VAZyz5IvUt3g1lMuNb8GvgE6fFMvIOA="] [type elliptic-curve-key]]
   [sink
    [[bytes hash] [shield dip] decorated
     float [verify] dip
     [[]]
     branch]
    [discard discard sink verify]
    [clone] dipdown branch]
   [[]] recover]
#+end_src
