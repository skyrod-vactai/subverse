...
Telepathyk, offering a user-friendly programming interface and the
...
With Telepathyk, you always know with maximum confidence, who created
...
In the telepathyk protocol, authentication is automatic, programmable
and flexible to balance the goals of security and convenience. 

In telepathyk, you will never need to:
...
In telepathyk's trusted environment, the presentation of content is
...
*** Serverless
Because Telepathyk offers strong authentication, privacy, and
...
Telepathyk automatically creates new identities when needed. For
...
Telepathyk will use a fresh identity. It's only when you need someone
to remember you, or be able to reach you later, that telepathyk will
re-use identities.
...
Telepathyk differs is that peers who don't care about the content also
...
who's who. Like nearly anything else in telepathyk, address book
...
#+begin_src kcats
  ...
  [[authenticate [[[[emit bytes hash] dip =]
                   [drop [] [inject] lingo first] ;; use functional before [inject] to turn off debug mode
                   [drop drop []] if]
                  [[]] recover]]
   [make-simple-script [[secret] unassign [first] sort wrap [sink verify] join]]
   [delegated-script [dump [secret] unassign [first] sort
                      [sink ;; cssig cs pk args
                       [emit bytes hash] [shield dip] decorated ;; css csh cs pk args
                       float ;; cs css csh pk args
                       [verify] dive ;; csh cs args
                       [authenticate] bail] swap prepend]]]
  ;[["hi"] [emit bytes hash] shield dump [] sink authenticate]
  ["bar" bytes key ;; child key
   "we attack at dawn" bytes ;; message
   [sign] shield ;; sign it
   ;swap
   swap  pair swap
   make-simple-script [emit bytes hash] shield ;; csh cs
   "foo" bytes key swap [sign] shielddown ;; sig pk
...
#+RESULTS:
#+begin_src kcats
[[[secret #b64 "LCa0a2j/xo/5m0U8HTBBNBNCLXBkg7+g+YpeiGJm564="] [public #b64 "NNJledu0Vmk+VAZyz5IvUt3g1lMuNb8GvgE6fFMvIOA="] [type elliptic-curve-key]] #b64 "ordodvDX8kNhFllnC++7Y5eELx9pF3hsuIH2+lkgmrqF2HuqJ/nEN0ru2mChKk1naYrBW2VA2gfmCCPvkdn9BQ==" [[[public #b64 "zKRbQGrYhpl8NNhvNBshIVOhzIE+OtmZo+FRboLoF4E="] [type elliptic-curve-key]] sink verify] [#b64 "wX8EhVgU6RLipJ77haemv9ISE3UDosCv/OUGYCKWPEiyRxCEMyAnzZGZ9hRI+MUmloTXbk/gx9h0KGWUxkGjDA==" #b64 "d2UgYXR0YWNrIGF0IGRhd24="]]
"we attack at dawn"
#+end_src
